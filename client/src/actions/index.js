...
export function getBooks(
    ...
    const request = axios.get(`/api/book?skip=${start}&limit=${limit}&order=${order}`)
                        .then(response => {
                            if(list)
...
export function deleteBook(id){
    let request = axios.delete(`/api/book_delete?id=${id}`)
                    .then( response => response.data)
...
export function registerUser(user,userList){
    ...
    return (dispatch) =>{
        request.then(({data}) => {
            let users = data.status ? [...userList,data.user] :userList
            let response = {
                success: data.status,
                users
            }
